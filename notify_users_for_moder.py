...
async def choose_year_for_group_setup(callback: CallbackQuery):
    ...
    selected_group = await db.select([db.func.distinct(ScheduleGroupTypeDay.group_name)]) \
            .where(ScheduleGroupTypeDay.faculty == GLOBAL_FACULTY).gino.all()
...
async def send_notification_for_faculty(callback: CallbackQuery, callback_data: dict):
    ...
    group_query = await db.select([db.func.distinct(ScheduleGroupTypeDay.group_name)]).where(ScheduleGroupTypeDay.faculty == GLOBAL_FACULTY).gino.all()
